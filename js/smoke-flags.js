/**
 * RUE2 — smoke flags (local only), RUE2 model.
 * Capture duds in context → list → copy for agent. Does not edit pack JSON.
 */

const KEY = "rue-exp-smoke-flags";
...
export function formatFlagsForAgent() {
  ...
  const lines = [
    `RUE v0.2 smoke flags · ${list.length} item(s) · ${new Date().toISOString().slice(0, 10)}`,
    `Process P0 first. Workspace: rue-exp. Fix accepts/cues/UI only — no drive-by refactors.`,
    "",
  ];
  list.forEach((f, i) => {
    lines.push(
