...
| **B8** | The gap is THIS unit's teaching point, not a neighbour's. Do not gap the infinitive particle `to` alone (`She wants ____ work` → `to` — contrast *to V* with bare *V*). Do not gap *a/an* inside *there is*. Do not leave dummy *There* already on the page and only test *is/are*. Do not gap *can/can't* when the error is *can to swim*. | `a1_like_want_need` 2026-08-29: *She wants ____ work*. `a1_there_is` 2026-08-29: *There is ____ university*. `a1_can` 2026-08-29: *I can ____* → swim / to swim, not another *can/can't* click. | `confirmed` — lint `toparticle`, `articlegap`, `theregap` |
