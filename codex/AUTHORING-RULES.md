...
| **Type** | Same target as Quiz · A8 contractions · I7 leftover `accepts` after a rename · explanations name THIS item (D1), not a neighbour (F2) · **B11:** if the English stem does not name the verb, `(lemma)` on the gap (`You ____ when I called.` → `(sleep)`) |
...
| **B11** | Whole-VP Type/Quiz: if the English stem does not name the verb, put `(lemma)` on the gap. | `a2_past_continuous` 2026-08-29: *You ____ when I called.* → *were sleeping*. *"should have verb needed in brackets."* | `enforced` — lint `verbcue` |
