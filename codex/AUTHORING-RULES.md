...
| **Quiz** | A0 one degree of freedom · **A9:** Czech past does not pick present perfect — *just/already/yet/never/for* (and Czech *už/ještě/právě*) · B3 authored chips on a form pack · **B4:** the unit's own error is a chip · **B19:** *(maybe) / (sure) / (impossible)* is not a verb lemma — author chips · B6 on-axis · **B18:** to-V vs -ing chips are the two real forms (*to read* / *reading*), never *to reading* or the bare stem as the only other chip · B7 sentence gap, not `____ = meaning` · B8 gap is THIS unit's point (not a neighbour, not *a/an* inside *there is*, not *can/can't* when the error is *to swim*) · **B21:** word-order Quiz is *Which is correct?* with three full sentences, not a gapped fragment · chips include the right answer · no untaught extras (C11) |
| **Type** | Same target as Quiz · A8 contractions · I7 leftover `accepts` after a rename · explanations name THIS item (D1), not a neighbour (F2) · **B7:** word-formation is a gapped **sentence** + capital root, never `person: ____` · **B11:** if the English stem does not name the verb, `(lemma)` on the gap (`You ____ when I called.` → `(sleep)`; `I ____.` → `(just/finish)`; `I like ____.` → `(coffee/very much)`; *the plan is* → `(plan / be)`) · **A11:** many-pair form pack cues the stem (`(surprise)`) so Type is not a vocab test · **E8:** word-order Type cloze cannot force the subject — skip Type |
| **Use** | F3: production may not lead. Partner + glue + this unit's words. A1 Czech is fine to smoke. Flag translation-only false-wrongs. **D5:** a stative in this unit names present simple in the explanation. **E4:** a sentence a teacher has never heard is not Use (*She wasn't listening when I came*). **E6:** the situation must be true in the world (cold → open a window is false). **E7:** A2/B1 form-pack Use is error-correction, not CZ→EN, when translation lets extra errors in. **E9:** do not rewrite a sentence that is already correct English — the Use prompt must contain an error. |

A1 translation is allowed. A2/B1 form-pack Use is E7. Recognition may lead; Use may not (F3).
...
| **C20** | An intro card teaches the student, not the lesson plan. No practice-sequence notes. Cut *What you practise* / *Co nácvičuješ* — that is a recap of the drill, not a teaching card. | `a1_object_pronouns` 2026-08-29: *First: match subject form… Then: full sentences…* / *Practice block* — *"teacher notes on this page: not good"*. `b1_modals_speculation` 2026-08-30: *"page not needed: cut it"*. | `confirmed` — lint `practisenote` |
