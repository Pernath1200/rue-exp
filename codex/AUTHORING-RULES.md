...
| **B9** | Twelve Match pairs is for **words and short phrases**. Full-sentence boards are **8**. Grammar sizes from a `.?!` or four-plus words. Vocab frames count **four-plus English words** (so *Hello.* stays a word board). Sentence tiles also need readable type (~0.88rem), not 0.76rem packed into 12 rows. | `a2_first_conditional` 2026-08-20: *"full sentences, we only need 8, not 12"*. `a1_some_any` 2026-08-29: char-avg 23 still painted 12 and felt walltexty. `trunk_verbs_more_a1` 2026-08-30: 12 full sentences — *"the matching screen is small and I have to squint"*. | `enforced` — engine `matchBoardSize` (grammar + vocab) |
