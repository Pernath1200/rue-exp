...
| **Quiz** | A0 one degree of freedom · **A9:** Czech past does not pick present perfect — *just/already/yet/never/for* (and Czech *už/ještě/právě*) · B3 authored chips on a form pack · **B19:** *(maybe) / (sure) / (impossible)* is not a verb lemma — author chips · B6 on-axis · **B18:** to-V vs -ing chips are the two real forms (*to read* / *reading*), never *to reading* or the bare stem as the only other chip · B7 sentence gap, not `____ = meaning` · B8 gap is THIS unit's point (not a neighbour, not *a/an* inside *there is*, not *can/can't* when the error is *to swim*) · chips include the right answer · no untaught extras (C11) |
| **Type** | Same target as Quiz · A8 contractions · I7 leftover `accepts` after a rename · explanations name THIS item (D1), not a neighbour (F2) · **B11:** if the English stem does not name the verb, `(lemma)` on the gap (`You ____ when I called.` → `(sleep)`; `I ____.` → `(just/finish)`; `I like ____.` → `(coffee/very much)`) · **A11:** many-pair form pack cues the stem (`(surprise)`) so Type is not a vocab test |
| **Use** | F3: production may not lead. Partner + glue + this unit's words. A1 Czech is fine to smoke. Flag translation-only false-wrongs. **D5:** a stative in this unit names present simple in the explanation. **E4:** a sentence a teacher has never heard is not Use (*She wasn't listening when I came*). **E6:** the situation must be true in the world (cold → open a window is false). **E7:** A2 form-pack Use is error-correction, not CZ→EN, when translation lets extra errors in. |
...
| **B11** | Whole-VP **or multi-word slot** Type/Quiz: if the English stem does not name the pieces, put them in brackets. Time-word VPs: `(just/finish)`. Position chips (*always drink* / *drink always*): `(always/drink)`. Bare *I like ____.* for *coffee very much* is a Type guess. | `a2_past_continuous` 2026-08-29: *You ____ when I called.* → *were sleeping*. `a2_present_perfect` 2026-08-29: *I ____.* → *have just finished* needs `(just/finish)`. `a2_will_going_to` 2026-08-29: *We ____ the kitchen.* → *are going to paint* needs `(paint)`. `a2_adverbs_order` 2026-08-29: *I like ____.* → *coffee very much* — *"maybe needs some clues in brackets - same for other questions"*. | `enforced` — lint `verbcue` |
...
| **E7** | A2 Use on a form pack is sentence **error-correction** (`use_mode: correct`), not CZ→EN translation, when the Czech lets extra errors in that are not the teaching point. One error per sentence (E2). Do not derive a wrong that is real English (G4: *She is worrying about the exam*; *I lived in Brno* is not a wrong for *I used to live in Brno*). | `a2_ed_ing_adjectives` 2026-08-29: *"I am wondering about doing direct translation at a2 / the more complex cz allows for more errors."* `a2_used_to` 2026-08-29: *"lots of errors can creep in due to translation / maybe just make this a full sentence error correction"*. | `confirmed` |
